...
namespace EshoppingV2._0.Controllers
{
    public class ShippingAddressesController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,location,state,pincode,phoneNumber")] ShippingAddress shippingAddress)
        {
            if (ModelState.IsValid)
            {
                var user = User.Identity;
                shippingAddress.userId = Guid.Parse(user.GetUserId());
